...
// server recomputes every submission from (timeSec, mistakes) and stores its own
// answer. That plus a floor on run time kills the obvious forgeries; it is not
// proof against a patient forger, and it isn't meant to be.
...
// Floor: 31 hand placements, each with a tool pick and a camera move. Nobody
// beats this honestly.
const MIN_SEC = 95;
const MAX_SEC = 7200;
...
async function topRows(env, limit, playerId) {
  const { results } = await env.DB.prepare(
    `SELECT player_id, name, score, time_sec, mistakes FROM scores
      ORDER BY score DESC, time_sec ASC LIMIT ?1`
  ).bind(limit).all();
...
async function countFor(env) {
  const row = await env.DB.prepare('SELECT COUNT(*) AS n FROM scores').first();
  return row ? row.n : 0;
}
...
async function rankFor(env, playerId) {
  if (!playerId) return null;
  const mine = await env.DB.prepare(
    'SELECT name, score, time_sec, mistakes FROM scores WHERE player_id = ?1'
  ).bind(playerId).first();
  if (!mine) return null;
  const ahead = await env.DB.prepare(
    `SELECT COUNT(*) AS n FROM scores
      WHERE score > ?1 OR (score = ?1 AND time_sec < ?2)`
  ).bind(mine.score, mine.time_sec).first();
  return {
...
async function rankForRun(env, score, timeSec) {
  const ahead = await env.DB.prepare(
    `SELECT COUNT(*) AS n FROM scores
      WHERE score > ?1 OR (score = ?1 AND time_sec < ?2)`
  ).bind(score, timeSec).first();
  return (ahead ? ahead.n : 0) + 1;
}

async function boardPayload(env, limit, playerId, extra) {
  const [board, total, you] = await Promise.all([
    topRows(env, limit, playerId),
    countFor(env),
    rankFor(env, playerId),
  ]);
  return { board, total, you, ...extra };
}
...
async function getBoard(url, env, headers) {
  ...
  return json(await boardPayload(env, limit, playerId), 200, headers);
}
...
async function postScore(req, env, headers) {
  ...
  if (!(timeSec >= MIN_SEC && timeSec <= MAX_SEC)) {
  ...
  // Keep the player's best run. A worse run leaves the row (and its name) alone.
  await env.DB.prepare(
    `INSERT INTO scores
       (player_id, name, score, time_sec, mistakes, dial_style, created_at)
  ...
  ).bind(playerId, name, score, timeSec, mistakes, dialStyle, now).run();

  const payload = await boardPayload(env, BOARD_LIMIT, playerId, {
    nameAdjusted: adjusted,
    runRank: await rankForRun(env, score, timeSec),
  });
